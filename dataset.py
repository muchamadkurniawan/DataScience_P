...
class class_dataset:
    ...
    def pickDataset(self):
        ...
        elif self.name =="asmaDataset":
            self.asmaDatasset()


    def asmaDatasset(self):
        print("ASMA DATASET")
        self.df = pd.read_csv("data_ga.csv")
    ...
    def artificialDataset(self):
        print("DATASET 2D")
        self.df = pd.read_excel(open('dataset_artificial.xlsx', 'rb'))
        X = pd.DataFrame(self.df, columns=(['x', 'y']))
        self.X = np.array(X)
        y = pd.DataFrame(self.df, columns=(['T']))
        y = np.array(y)
        yy = []
        for i in range(len(y)):
            yy.append(y[i][0])
